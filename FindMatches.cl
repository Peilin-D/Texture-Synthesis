            ...
            const int halo)
{
    ...
    if (index < buf_w) {// From load_halo.cl
        for (int row = 0; row < buf_h; row++) {
            buffer[row * buf_w + index] = gpu_texture[get_index(width, height, buf_corner_x + index, buf_corner_y + row)];
        }
    }
    ...
    if ((x < width) && (y < height)) {
        for(int row = buf_x-(window_width-1)/2; row<buf_x-(window_width-1)/2 + window_width; row++){
            for(int col=buf_y-(window_width-1)/2; col<buf_y-(window_width-1)/2 + window_width;col++){
                match_list[y*width+x]=buffer[get_index(buf_w, buf_h, col, row)]-template[col-offset_x, row-offset_y];
            }
        }
    }
}
